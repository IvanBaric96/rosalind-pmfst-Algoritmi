...
def Score(Peptide, Spectrum):  ###radi
  cyclo=Cyclospectrum(Peptide)
  rj=dict()
  skup=set(cyclo)
  for x in skup:
    rj[x]=KolikoPuta(x,cyclo)

  skup_spectrum=set(Spectrum)
  rj_spectrum=dict()
  for x in skup_spectrum:
    rj_spectrum[x]=KolikoPuta(x,Spectrum)
  
  cnt=0
  for x in rj_spectrum.keys():
    if x in rj.keys():
      if rj[x]>rj_spectrum[x]:
        cnt+=rj_spectrum[x]
      else:
        cnt+=rj[x]
  return cnt
...
pep=""
spectrum=[]
#print("Result: "+str(Score(pep,spectrum)))
###